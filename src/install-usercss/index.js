...
(async function init() {
  ...
  if (fsh) {
    ...
    firstGet = API.usercss
      .getInstallCode(initialUrl)
  ...
  try {
    ...
    ({ dup, style } = await API.usercss.build({
      sourceCode,
      checkDup: true,
      metaOnly: true,
    }));
    ...
    (!dup
      ...
      (ok) =>
        ...
        API.usercss
          ...
          .then(install)
          .catch((err) =>
            messageBox.alert(t("styleInstallFailed", err.message || err), "pre")
          )
    );
  };
  ...
  const updateUrl = tryURL(
    style.updateUrl || initialUrl || (dup && dup.updateUrl)
  );
  if (!updateUrl) {
...
function updateMeta(newStyle) {
  ...
  if (dup && dup.updateUrl) {
    $(".set-update-url").title = t("installUpdateFrom", dup.updateUrl).replace(
      /\S+$/,
      "\n$&"
    );
  }
  ...
  function makeExternalLink() {
    ...
    return (
      (data.homepageURL || data.supportURL) &&
      $create("div", [
        $create("h3", t("externalLink")),
        $create(
          "ul",
          urls
            .map((args) => args && $create("li", $createLink(...args)))
            .filter(Boolean)
        ),
      ])
    );
  ...
  if (err[0]) {
    ...
    $(".warnings").appendChild(
      $create(".warning", [
        ...
        "\n",
        ...err.map((e) =>
          e.message ? $create("pre", e.message) : e || "Unknown error"
        ),
      ])
    );
  }
...
async function getAppliesTo() {
  ...
  for (const section of style.sections) {
    const targets = [].concat(
      ...TARGETS.map((_) => section[_]).filter(Boolean)
    );
    res.push(...targets);
...
function initLiveReload() {
  ...
  return (e) => {
    ...
    if (liveReloadEnabled) start({ force: true });
    else stop();
  ...
  async function start(opts) {
    if (
      fsh &&
      (fso ||
        ((fso = global.FileSystemObserver) &&
          (fso = new fso(() => debounce(check, 20)))))
    ) {
      try {
