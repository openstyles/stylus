...
export const clientData =
  ...
  (__.MV3
    ...
    : API.setClientData({ url: location.href, dark: isCssDarkScheme() }).then(
        (data) => {
...
const defaults = {
  ...
  "editor.livePreview.delay": 0.2, // seconds (Chrome devtools uses 200ms)
...
export const set = (key, val, isSynced) => {
  ...
  if (!global[k_busy] || !__.IS_BG)
    onChange[key]?.forEach((fn) => fn(key, val));
  if (!isSynced && !__.IS_BG)
    (toUpload ??= Promise.resolve().then(upload) && {})[key] = val;
