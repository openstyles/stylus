...
export async function renderFavs(container = installed) {
  ...
  const regexpRemoveNegativeLookAhead =
    /(\?!([^)]+\))|\(\?![\w(]+[^)]+[\w|)]+)/g;
  // replace extra characters & all but the first group entry "(abc|def|ghi)xyz" => abcxyz
  const regexpReplaceExtraCharacters = /[\\(]|((\|\w+)+\))/g;
  const regexpMatchRegExp =
    /[\w-]+[.(]+(com|org|co|net|im|io|edu|gov|biz|info|de|cn|uk|nl|eu|ru)\b/g;
  ...
  for (const target of container.$$(".target")) {
    ...
    const img =
      target.$(":scope > img:first-child") ||
      target.insertAdjacentElement(
        "afterbegin",
        $create("img", { loading: "lazy" })
      );
...
export function renderMissingFavs(num, iconsMissing, iconsEnabled) {
  for (const entry of installed.children) {
    entry
      .$(".applies-to")
      .classList.toggle("has-more", entry._numTargets > num);
    if (
      !entry._allTargetsRendered &&
      num > entry.$(".targets").childElementCount
    ) {
