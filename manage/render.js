...
define(require => {
  ...
  const {filterAndAppend} = require('./filters');
  ...
  const render = {
    ...
    createStyleElement({style, name: nameLC}) {
      ...
      render.createStyleTargetsElement({entry, style});

      return entry;
    },

    createStyleTargetsElement({entry, expanded, style = entry.styleMeta}) {
      const parts = elementParts;
    ...
    showStyles(styles = [], matchUrlIds) {
      ...
      function renderStyles() {
        ...
        while (index < sorted.length && (shouldRenderAll || performance.now() - t0 < 20)) {
          ...
          const entry = render.createStyleElement(info);
        ...
        if (index < sorted.length) {
          requestAnimationFrame(renderStyles);
          if (firstRun) setTimeout(render.getFaviconImgSrc);
          firstRun = false;
          return;
        }
        setTimeout(render.getFaviconImgSrc);
    ...
    switchUI({styleOnly} = {}) {
      ...
      if (changed.enabled || (iconsMissing && !elementParts)) {
        ...
        API.styles.getAll().then(render.showStyles);
        return;
      }
      ...
      if (changed.targets) {
        for (const entry of installed.children) {
          ...
          if (!entry._allTargetsRendered && newUI.targets > $('.targets', entry).childElementCount) {
            render.createStyleTargetsElement({entry, expanded: true});
            iconsMissing |= iconsEnabled;
          }
        }
      }
      if (iconsMissing) {
        debounce(render.getFaviconImgSrc);
        return;
      }
    },
  };
