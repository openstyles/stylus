...
define(require => {
  ...
  const {$entry} = require('./render');
  const {filtersSelector, filterAndAppend} = require('./filters');

  const updaterUI = {

    checkUpdate(entry, {single} = {}) {
  ...
  async function showUpdateHistory(event) {
    ...
    let deleted = false;
    const {chromeLocal} = await require(['/js/storage-util']);
